...
const GetBookings = () => {
    ...
    useEffect(() => {
        ...
        fetch(`${process.env.REACT_APP_BE_BASE_URL}/api/get_bookings?id=${encodeURIComponent(localStorage.getItem('id'))}`, {
            method: 'GET',
            headers: {
    ...
    const removeBooking = (id) => {
        console.log('id', id);
        fetch(`${process.env.REACT_APP_BE_BASE_URL}/api/delete_bookings/${id}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `${localStorage.getItem('token')}`,
                // 'Content-Type': 'application/json',
